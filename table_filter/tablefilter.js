/*====================================================
	- HTML Table Filter Generator v1.6.1
	- By Max Guglielmi
	- mguglielmi.free.fr/scripts/TableFilter/?l=en
	- please do not change this comment
	- don't forget to give some credit... it's always good for the author
	- Special credit to Cedric Wartel and 
	cnx.claude@free.fr for contribution and inspiration
=====================================================*/

// global vars
var TblId, SearchFlt, SlcArgs;
TblId = new Array(), SlcArgs = new Array();
filterWidth = new Array();
colWidth = new Array();
...
jQuery(document).ready(function() {
	...
	if ((ieVersion > -1) && (ieVersion < 9)) {
		jQuery('.filter').mousedown(function(){
			if (jQuery(this).is("select")) {
				var prewidth = jQuery(this).outerWidth();
				jQuery(this).css('width','auto').css('position','absolute').css('top','3px');
				var postwidth = jQuery(this).outerWidth();
				if (prewidth < postwidth)
					jQuery(this).parent().next().children().hide();
				}
			});
		jQuery('.filter').change(function(){
			if (jQuery(this).is("select") ) jQuery(this).blur();
			});
		jQuery('.filter').blur(function(){
			if (jQuery(this).is("select") ) {
				jQuery(this).parent().next().children().show();
				var childNum = jQuery(this).parent().index();
				jQuery(this).css('width',filterWidth[childNum]).css('position','relative').css('top','0');
				}
			});
		}
	});
		...
		if(arguments.length>1) {
			for(var i=0; i<arguments.length; i++) {
				...
				switch(argtype.toLowerCase()) {
					case "number": ref_row = arguments[i];
					break;
...
function AddGrid(id) {
	...
	f!=undefined && f["alternate_rows"]==false ? alternateBgs=false : alternateBgs=true;//enables/disbles rows alternating bg colors
	...
	if(fltgrid) {
		var fltrow = t.insertRow(1); //adds filter row
		fltrow.className = "fltrow";
		for(var i=0; i<n; i++) { // this loop adds filters
			var fltcell = fltrow.insertCell(i);
			fltcell.className = "filtercell";
			//fltcell.noWrap = true;
			i==n-1 && displayBtn==true ? inpclass = "flt_s" : inpclass = "flt filter";
			if(f==undefined || f["col_"+i]==undefined || f["col_"+i]=="text" || f["col_"+i]=="none") {
				var inptype;
				(f==undefined || f["col_"+i]==undefined) ? inptype="text" : inptype="hidden";//show/hide input
				var inp = createElm( "input",["id","flt"+i+"_"+id],["type",inptype],["class",inpclass],["placeholder","enter search terms"] );
	...
	if(colWidth) {
		...
		setColWidths(id);
		filterWidth = t.tf_colWidth;
		var j = filterWidth.length;
		var start_row = parseInt( t.tf_ref_row );//filter start row
		var ncells = getCellsNb(id,start_row);
		for(var i=0; i<=filterWidth.length-1; i++) {
			filterWidth[i] = parseInt(filterWidth[i],10) + 10;
			if (i == (filterWidth.length - 1))
				filterWidth[i] = parseInt(filterWidth[i],10) + 6;
			filterWidth[i] = filterWidth[i] + "px";
			jQuery('#flt' + i + '_' + id).css('width',filterWidth[i]);
			}//for i
		}
	...
	if(bindScript) {
		...
		if( t.tf_bindScript!=undefined && t.tf_bindScript["target_fn"]!=undefined ) { //calls a fn if defined
...
=====================================================*/
	if( hasGrid(id) ) {
		...
		colWidth = t.tf_colWidth;
		var start_row = parseInt( t.tf_ref_row );//filter start row
		var ncells = getCellsNb(id,start_row);
		// colWidth[0] = parseInt(colWidth[0],10) - 1;
		// colWidth[colWidth.length - 1] = parseInt(colWidth[colWidth.length - 1],10) + 10;
		for(var i=0; i<=colWidth.length-1; i++) {
			colWidth[i] = parseInt(colWidth[i],10) - 20;
			colWidth[i] = colWidth[i] + "px";
			for(var k=0; k<ncells; k++) {
				if(k==i) {
					jQuery('#' + id + ' tr:first th:eq(' + k + ')').css('width',colWidth[i]);
					}
			}//var k
		}//for i
	}//if hasGrid
...
=====================================================*/
{
	...
	chars = new Array('\\','[','^','$','.','|','?','*','+','(',')','&','"',"'",'http://www.cisco.com/','<','>');
