...
function KofC_ChooseCSS(pc_css,mac_css) {
    ...
    if (navigator.userAgent.toLowerCase().indexOf('mac_ppc') != -1) { // AOL 5, MAC OS 9
      ...
      if (! pc_css) { // No parameters passed
        stylesheet = "../styles/knights-mac.css";
      }
...
function KofC_getLanguageLink(currentLanguageCode, newLanguageCode, coordinates, altText) {
  ...
  var domainStr;
  var newHRef;
  domainStr = location.href.split('.org')[0] + '.org';
  newHRef = domainStr + '/' + newLanguageCode + '/index.html';
  
  var commonString = ' onClick="KofC_setCookie(\'' +
      KOFC_COOKIE + '\',\'' + LANGUAGE_KEY + '=' + newLanguageCode + '#\',\'' +
      KofC_cookieExpiration(KOFC_COOKIE_DURATION_IN_DAYS) + '\');"'
    
  document.write('<a href="' + newHRef + '"' + commonString + ' class="UtilityLink">' + newLanguageName + '</a>');
...
function KofC_forwardUser(fURI) {
	...
	bm.setAttribute('name','msvalidate.01');
	bm.setAttribute('content','F6138752694FD736F4A34A0703F58791');
	var bmh = document.getElementsByTagName('head')[0];
	bmh.appendChild(bm);
...
function KofC_redirect(fURI) {
  if(fURI.match("monthly_report"))
  {
	  window.location = "http://www.kofc.org/en/knightsinaction/"+fURI;
  } else {
